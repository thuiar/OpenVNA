...
logger = set_logger(log_dir=Path(Path(__file__).parent, "logs"), verbose_level=1)
...
def real_noise(
    ...
    background_noise_path = Path(__file__).parent / "assets" / "noise" / "background"
    sudden_noise_path = Path(__file__).parent / "assets" / "noise" / "sudden"
    reverb_path = Path(__file__).parent / "assets" / "noise" / "IRs"
