...
class TFR_NET(nn.Module):
    def __init__(self, args):
        ...
        self.text_model = BertTextEncoder(finetune=args.use_bert_finetune)
    ...
    def forward(self, text, audio, vision):
        text, audio, vision = text[0], audio[0], vision[0]
        text_mask = text[:,1,:]
        text = self.text_model(text)
        text_h, audio_h, vision_h, text_h_g, audio_h_g, vision_h_g = self.align_subnet(text, audio, vision)
        prediction = self.fusion_subnet((text_h, text_mask), (audio_h, text_mask), (vision_h, text_mask))
        return prediction
